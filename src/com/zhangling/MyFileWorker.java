...
public class MyFileWorker {
	...
	public void deleteFile(String deleteFile) {
		...
		driver.findElement(By.className("btn_primary_large")).click();
		Utils.waitFor(3000);
		Boolean fileExist = Utils.isExists(driver, By.xpath("//a[@data-name='"+deleteFile+"' and @title='"+deleteFile+"']"));
		
		if (fileExist) {
			System.out.println("删除文件失败");
			Assert.fail("删除文件失败");
		}
		driver.findElement(By.xpath("//span[text()='回收站']")).click();	
		Boolean fileExistInCycle = Utils.isExists(driver, By.xpath("//a[@title='"+deleteFile+"']"));
		if(!fileExistInCycle){
			System.out.println("删除文件失败");
			Assert.fail("删除文件失败");
		}else{
	...
	public void restore(String fileName){
		...
		deleteFile(fileName);
		driver.findElement(By.xpath("//span[text()='回收站']")).click();
		driver.findElement(By.xpath("//ul[@data-name='"+fileName+"']/li[1]/input")).click();
		driver.findElement(By.id("restore")).click();
		Utils.waitFor(3000);
		Navigate.toMyFile(driver);
		Boolean file = Utils.isExists(driver, By.xpath("//a[@title='"+fileName+"']"));
		if(!file){
